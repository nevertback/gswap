...
let myroot = rootpath,
	mypath = {		
        ...
        v4:        rootpath + 'v4origin/',            //js目录
        v4min:     rootpath + 'v4/',         //js输出目录
...
gulp.task('v4', function() {
    return gulp.src(mypath.v4+'*.js')
        .pipe(sourcemaps.init())
        .pipe(plumber({errorHandler: notify.onError('Error: <%= error.message %>')}))
        .pipe(concat('gamersky.mobile.v4.js'))
        .pipe(gulp.dest(mypath.v4min))
        .pipe(rename({suffix: '.min'}))
        .pipe(uglify())
...
gulp.task('develop', function() {
	...
	gulp.watch([mypath.v4 + '*.js'], ['v4']);
	gulp.watch([mypath.js + '*.js'], ['jsmin']);
	gulp.watch([mypath.tpl + "*.html",mypath.tpl + "*/*.html"], ['fileinclude']);
